...
int main(int argc, char* argv[])
{
    ...
    int imagenumber = 0;
    while (imagenumber < numBoards)
    {
        //cap1 >> img1;
        //cap2 >> img2;
        stringstream s; // stringstream used for the conversion
        s << imagenumber++;
        img1 = imread("darpan_calibration/cam--0-"+s.str()+".tiff",CV_LOAD_IMAGE_COLOR);
        img2 = imread("darpan_calibration/cam--1-"+s.str()+".tiff",CV_LOAD_IMAGE_COLOR);
        resize(img1, img1, Size(320, 280));  //IS THIS REQUIRED?? IF SO, WHAT SIZE?
        resize(img2, img2, Size(320, 280));
        cvtColor(img1, gray1, CV_RGB2GRAY);
        //printf("cvtColor for img1 done");
        cvtColor(img2, gray2, CV_RGB2GRAY);
        //printf("cvtColor for img2 done");
        printf("reading images done\n");
        ...
        if (found1 && found2)
        {
            printf("Both found. Press any key. Successes = %d \n",success);
            //k = waitKey(0);  THIS CAUSES problems. 
            k=' ';
        }
        if (k == 27)
        {
    ...
    FileStorage fs1("pointgrey_stereocalib.yml", FileStorage::WRITE);
    fs1 << "CM1" << CM1;
    fs1 << "CM2" << CM2;
    fs1 << "D1" << D1;
    fs1 << "D2" << D2;
    ...
    {    
        ...
        stringstream s; // stringstream used for the conversion
        s << count;
        img1 = imread("darpan_calibration/cam--0-"+s.str()+".tiff",0);
        img2 = imread("darpan_calibration/cam--1-"+s.str()+".tiff",0);
